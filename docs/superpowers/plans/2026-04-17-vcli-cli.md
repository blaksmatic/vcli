...
/// timestamps.
#[must_use]
pub fn format_unix_ms(t: UnixMs) -> String {
    // Constants for Gregorian calendar math.
    let ms = t.as_millis();
...
mod tests {
    ...
    fn format_unix_ms_epoch_is_1970_01_01() {
        let s = format_unix_ms(UnixMs::from_millis(0));
    ...
    fn format_unix_ms_known_instant() {
        ...
        let s = format_unix_ms(UnixMs::from_millis(ms));
        assert_eq!(s, "2026-04-16T19:42:11.123Z");
    }
}
```

**Note on `UnixMs`:** `vcli_core::clock::UnixMs` exposes `from_millis(i64)` and `as_millis() -> i64`. If the constructor is named differently in the current crate (`as_ms`/`new`), substitute — `cargo check` will tell you. The Howard Hinnant formula is plug-and-play.
...
pub async fn run<W: Write>(
    ...
    while let Some(frame) = stream.next_frame().await? {
        if let Some(ev) = frame.event.as_ref() {
            if since_ms.is_some_and(|t| ev.at.as_millis() < t) {
                continue;
            }
...
**Note on `Event.at`:** the test assumes `UnixMs::from_millis(i64) -> UnixMs` and `UnixMs::as_millis() -> i64`. If the actual API is `UnixMs(pub i64)` or similar, adapt in-place.
