...
- **B1.** On `target_os = "macos"`, `default_runtime_factory` constructs `vcli_capture::macos::MacCapture::new()` and `vcli_input::macos::CGEventInputSink::new(kill)`. On every other platform, the factory keeps the v0 mocks. Windows backends arrive in v0.4 per Decision G of the 2026-04-16 review; this appendix does not bring them forward.
