//! Scenario: Click → WaitFor(postcondition) fails when the postcondition
//! never flips before `timeout_ms`.
...
#[test]
fn wait_for_postcondition_fails_when_never_truthy() {
    let red = ScriptedCapture::solid(1, 1, [255, 0, 0, 255]);
    let capture = Box::new(ScriptedCapture::new(vec![red.clone(), red.clone(), red]));
